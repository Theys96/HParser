...
followSetSubsets (Grammar rules) nonTerminal = S.unions (map (ruleFollowSubsets nonTerminal) rules)
   where
      ...
      ruleFollowSubsets nonTerminal (Rule a rh)
         ...
         | S.member "" (symbolFirstSet (Grammar rules) c)           -- ε in First{C}
...
directFollowSet2 (Grammar rules) nonTerminal = S.filter (/= "") $ S.unions (map checkRule rules) 
   where
      ...
      checkRule (Rule lh (nt:x:xs))
         ...
         | otherwise          = checkNextRule                   -- to the next symbol
            where
               ...
               firstSetC = ruleFirstSet (Grammar rules) c
               c = Rule lh (x:xs)
